...
def test_create_vacancy_success(client_as_admin):
    with patch(
        "app_backend.routers.api.vacancy.create_vacancy_command_handler"
    ) as mock_handler:
...
def test_list_vacancies_success(client_as_student):
    with patch(
        "app_backend.routers.api.vacancy.list_vacancies_command_handler"
    ) as mock_handler:
...
def test_update_vacancy_success(client_as_admin):
    with patch(
        "app_backend.routers.api.vacancy.update_vacancy_command_handler"
    ) as mock_handler:
        from app_backend.schemas.vacancy import VacancyResponse
...
def test_delete_vacancy_success(client_as_admin):
    with patch(
        "app_backend.routers.api.vacancy.delete_vacancy_command_handler"
    ) as mock_handler:
        mock_handler.return_value = MagicMock(
            success=True,
            got_error=lambda: False,
