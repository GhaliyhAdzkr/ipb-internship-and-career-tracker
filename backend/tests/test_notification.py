...
def test_list_notifications(client_as_student):
    with patch(
        "app_backend.routers.api.notification.list_notifications_command_handler"
    ) as mock_handler:
...
def test_get_unread_count(client_as_student):
    with patch(
        "app_backend.routers.api.notification.get_unread_count_command_handler"
    ) as mock_handler:
        mock_handler.return_value = MagicMock(got_error=lambda: False, count=5)
...
def test_read_notification(client_as_student):
    with patch(
        "app_backend.routers.api.notification.read_notification_command_handler"
    ) as mock_handler:
        from app_backend.schemas.notification import NotificationResponse
...
def test_delete_notification(client_as_student):
    with patch(
        "app_backend.routers.api.notification.delete_notification_command_handler"
    ) as mock_handler:
        mock_handler.return_value = MagicMock(got_error=lambda: False)
